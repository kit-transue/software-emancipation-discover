...
(if editor-is-xemacs
  ...
  (defun dislite-emacs-menu (substr)
    (setq res (x-popup-menu menu-pos dislite-popup-menu))
    (if (numberp res)
	(let* ((param (dislite-param)))
	  (if (= res 0) (dislite-info))
	  (if (= res 1) (dislite-open-def))
	  (if (= res 2) (dislite-query))
	  (if (= res 3) (dislite-instances))
	  (if (= res 4) (dislite-browser-popup)))))
  ...
  (global-set-key [M-mouse-1] 'dis-ignore-event)
  (global-set-key [M-down-mouse-1] 'dislite-rightclick-cb)
  (global-set-key [M-S-mouse-1] 'dis-ignore-event)
  (global-set-key [M-S-down-mouse-1] 'dislite-open-def)
)
...
(defun dislite-info ()
	(interactive)
	(dislite-send (concat "0 inst_def\t" (dislite-param))))

(defun dislite-open-def ()
	(interactive)
	(dislite-send (concat "0 open_def\t" (dislite-param))))

(defun dislite-query ()
	(interactive)
	(dislite-send (concat "0 query\t" (dislite-param))))

(defun dislite-instances ()
	(interactive)
	(dislite-send (concat "0 instances\t" (dislite-param))))

(defun dislite-browser-popup () 
	(interactive)
	(dislite-send "0 popup\n"))

(defun menuBar ()
	(require 'easymenu)
	(defvar crover-mode-map nil)

  (defun bar-make-selection ()
	...
	(defun bar-browser-popup () 
		(interactive)
		(bar-make-selection)
	  	(dislite-browser-popup))

	(defvar crover-dummy-map nil)
	(easy-menu-define crover-dummy-map (current-global-map) "DISCOVER menus" dislite-bar-menu))
...
(defvar dislite-process-name "emacsAdapter"
...
(defvar dislite-cmd-list '(("Emacs_Menu" . dislite-emacs-menu) 
						   ("version" . dislite-emacs-version)
						   ("open" . dislite-open-proc) 
						   ("openpos" . dislite-openpos-proc) 
						   ("openreadonly" . dislite-open-readonly-proc) 
						   ("info" . dislite-info-proc) 
						   ("Dislite_Error" . dislite-error-proc) 
						   ("Dislite_Exit" . dislite-exit) 
						   ("DeveloperXPress_Restart" . dislite-restart) 
						   ("Emacs_Exit" . dislite-emacs-exit) 
						   ("Dislite_Service" . dislite-service) 
						   ("Dislite_Loaded" . dislite-loaded) 
						   ("complete" . di-complete-cb) 
						   ("cm_get" . dislite-cm-cb) 
						   ("cm_unget" . dislite-cm-cb) 
						   ("cm_put" . dislite-cm-cb) 
						   ("cm_before_put" . dislite-before-cmput) 
						   ("editor_get_reply" . dislite-get-reply-cb))
...
(defun dislite-start () 
  ...
  (apply 'start-process
		   dislite-process-name 
		   " *dislite*"
		   (get-adapter-command)
		   )
;  (start-process dislite-process-name " *dislite*" command)
...
(defun dislite-emacs-version (substr)
    (setq long-ver (downcase (emacs-version)))
    (if (= (read (substring substr 1)) 1)
	(dislite-send (concat (number-to-string emacs-major-version) "." (number-to-string emacs-minor-version) "\n"))
	(dislite-send (concat long-ver "\n"))
    )
)
...
(defun dislite-open-proc (substr)
  ...
  (let ((pos1 (string-match "{.*}" substr))
	...
	(progn
	  ...
	  (setq tmpFile (car temp-string))
	  (if (not (stringp tmpFile))
      	(setq file (symbol-name tmpFile))
	  (setq file tmpFile))
	...
	(if (buffer-modified-p)
	  ...
	  (if (not editor-is-xemacs)
	      (let* ((pos (event-start event))
		     (posxy (posn-x-y pos)))
		(setq menu-pos (list (list (+ (car posxy) (if (< emacs-major-version 21) 50 20)) 
					 (+ (cdr posxy) (if is_unix (if (< emacs-major-version 21) 40 10) 10))) (posn-window pos)))))
	      (funcall 'dislite-emacs-menu 'cur_event)))))
